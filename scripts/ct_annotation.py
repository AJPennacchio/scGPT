...
hyperparameter_defaults = dict(
    seed=0,
    dataset_name="brain_atlas",
    do_train=True,
    load_model="../save/scGPT_brain",
    mask_ratio=0.0,
    epochs=1, # was 10
    n_bins=51,
    ...
    dab_weight=0.0,
    lr=1e-4,
    batch_size=32, # was 32; 512, 2048 too big, 128 good
    layer_size=128,
...
print("Finished Hyperparameter Setup")
...
if config.load_model is not None:
    model_dir = Path("models/scGPT_brain") #Path(config.load_model)
...
adata_test = adata[adata.obs["train_test_split"]==1]  #adata[adata.obs["str_batch"] == "1"]
adata = adata[adata.obs["train_test_split"]==0]  #adata[adata.obs["str_batch"] == "0"]
